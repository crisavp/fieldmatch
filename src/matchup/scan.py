"""`matchup scan`: inventory a campaign -- what is on disk, what is readable,
...
def var_cadences(ds):
    ...
    `tol` is the default matching tolerance collocation will use (half step).
    """
    t = ds["time"].values
    out = {}
    for v in ds.data_vars:
        ...
        if finite.size == 0:
            ...
            continue
        tv = t[finite]
        step = np.median(np.diff(tv)) if tv.size > 1 else None
        tol = step / 2 if step is not None else None
        ...
        if r["role"] == "model":
            ...
            if r.get("needs_lead"):
                L.extend(_wrap_text(
                    ...
                    "`match ... --lead 12-35` (a lead window across all inits) "
            ...
            for (step, tol, n), vs in sorted(groups.items(), key=lambda kv: -kv[0][2]):
                head = f"    every {step:<5} ({n:>4} steps, tol {tol:<5}) : "
