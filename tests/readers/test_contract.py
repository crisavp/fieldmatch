...
def test_prefix_keeps_extras_out_of_the_standard_namespace():
    ...
    from fieldmatch.collocate_track import DIRECTION_VARS
...
def test_choice_rejects_unknown_value_listing_the_alternatives():
    from fieldmatch.readers import _choice
...
def test_every_reader_declares_its_coordinate_sources():
    """Regression: `fieldmatch vars` listed CMEMS latitude/longitude/time as
    ...
    from fieldmatch import readers as r
    for kind in ("altimeter_cmems", "altimeter_s3", "altimeter_s6",
                 "sentinel1", "buoy_ispra"):
        src = inspect.getsource(r.READERS[kind].reader)
        for coord in ("lat_source", "lon_source", "time_source"):
            assert coord in src, f"{kind} does not declare {coord}"


def test_reader_preserves_units_through_quality_and_coastal_filter():
    from fieldmatch.readers import _open_sea_cloud, _qc_by_flag
    raw=xr.Dataset({'wave':('record',[100.,200.,300.],{'units':'cm'}),
                    'quality':('record',[0,1,0]),'distance':('record',[1e3,40e3,50e3])})
    vals,_,_=_qc_by_flag(raw,{'hs':raw.wave},[('hs','quality')])
    a=_open_sea_cloud(raw,np.array(['2026-01-01']*3,dtype='datetime64[ns]'),
                      [0,0,0],[0,0,0],vals,'distance','absent',30,False)
    assert a.hs.attrs['units']=='cm'
    np.testing.assert_equal(a.record_index.values,[1,2])
    assert np.isnan(a.hs.values[0])


def test_buoy_header_units_are_not_silently_replaced(tmp_path):
    from fieldmatch.readers import read_buoy_ispra
    p=tmp_path/'buoy.csv';p.write_text('time;hm0 [cm]\n2026-01-01;200\n')
    a=read_buoy_ispra(p,lat=0,lon=0)
    assert a.hs.attrs['units']=='cm'
