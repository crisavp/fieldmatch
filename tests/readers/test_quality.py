...
def test_open_sea_mask_rejects_land_and_coastal_strip():
    ...
    from fieldmatch.readers import _open_sea_mask
    src = xr.Dataset({
...
def test_counts_are_summed_across_files_not_inherited():
    ...
    from fieldmatch.campaign import combine_provenance
...
def test_partially_available_coastal_filter_is_explicit():
    from fieldmatch.readers import _open_sea_mask
    raw=xr.Dataset({'distance':('r',[50000.])})
    mask,description=_open_sea_mask(raw,'distance','surface',30,True)
    assert mask[0] and 'surface filter not applied' in description
